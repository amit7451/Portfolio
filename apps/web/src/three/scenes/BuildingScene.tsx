  ...
  for (let floor = 0; floor < FLOOR_COUNT; floor++) {
    ...
    elements.push(
      ...
      >
        ...
        <meshLambertMaterial color={ceramicColor} />
      </mesh>
    );
...
// ────────────────────────────────────────────────────
// VisibilityCullingFloor — Automatically hides floors not in view
// ────────────────────────────────────────────────────
function VisibilityCullingFloor({ floorIndex, children, position }: { floorIndex: number, children: React.ReactNode, position: [number, number, number] }) {
  const scroll = useScroll();
  ...
  useFrame(() => {
    if (groupRef.current) {
      const floorsToTravel = FLOOR_COUNT - 1;
      const currentFloor = scroll.offset * floorsToTravel;
      // Show floor if it's within 1.2 floors of current position
      groupRef.current.visible = Math.abs(currentFloor - floorIndex) <= 1.2;
    }
  });
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        >
          ...
          <Suspense fallback={null}>
              ...
              <Environment preset="studio" background={false} environmentIntensity={0.1} resolution={256} />
              ...
              <group>
                ...
                <VisibilityCullingFloor floorIndex={0} position={[0, 0, 0]}>
                  ...
                  />
                </VisibilityCullingFloor>

                {/* ══ FLOOR 1: Projects Room ══ */}
                <VisibilityCullingFloor floorIndex={1} position={[0, -FLOOR_SPACING, 0]}>
                ...
                <VisibilityCullingFloor floorIndex={2} position={[0, -2 * FLOOR_SPACING, 0]}>
                ...
                <VisibilityCullingFloor floorIndex={3} position={[0, -3 * FLOOR_SPACING, 0]}>
